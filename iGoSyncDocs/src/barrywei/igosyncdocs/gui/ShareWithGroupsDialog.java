/**
 * @(#)ShareWithGroupsDialog.java Jul 16, 2010
...
import barrywei.igosyncdocs.action.ShareWithGroupAction;
 ...
 * @since JDK1.6
 */
public class ShareWithGroupsDialog extends JDialog {
	...
	public ShareWithGroupsDialog(IGoSyncDocsMain frame) {
		super(frame);
		setIconImage(Toolkit.getDefaultToolkit().getImage(ShareWithGroupsDialog.class.getResource("/ch/randelshofer/quaqua/images/FileView.computerIcon.png")));
		try {
			this.frame = frame;
	...
	private void initComponents() {
		...
		textArea.setText(" Enter group email address and \n Split email address with ',' if you need.");
		
		//event handler
		btnClose.addActionListener(new DialogCloseAction(this, false));
		
		btnShare.addActionListener(new ShareWithGroupAction(this,frame.getRemoteTableSelectedItem()));
		
		textArea.addFocusListener(new FocusListener() {
			public void focusLost(FocusEvent e) {
				if(textArea.getText().length()==0) {
					textArea.setText(" Enter group email address and split email address with ',' if you need.");
				}				
			}
